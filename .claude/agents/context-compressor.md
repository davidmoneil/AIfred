...
## Preservation Manifest (JICM v3)

If a preservation manifest exists at `.claude/context/.preservation-manifest.json`, **USE IT**.

The manifest is generated by the PreCompact hook and contains AI-driven prioritization:
...
**Manifest Priority Levels**:
- `critical`: Include verbatim, no summarization
- `high`: Include with concise summary
- `medium`: Reference by path/name only
- `low`: Summarize aggressively or omit

## Workflow

1. **Check for manifest** - Read `.claude/context/.preservation-manifest.json` if exists
2. **Review /context baseline** - Understand what's consuming context
3. **Scan context** - Review the full conversation inherited from parent
4. **Apply manifest priorities** - Use manifest to guide preserve/summarize/drop decisions
5. **Categorize remaining** - Identify what falls into preserve/summarize/drop
6. **Compress** - Write concise versions of preserved content
7. **Validate** - Ensure nothing critical is lost
8. **Write** - Output to `.claude/context/.compressed-context.md`
9. **Report** - Return summary to caller
