 ...
 *
 * Event: PostToolUse
 * Triggers: After Read tool calls
 */
...
const ACCESS_LOG_FILE = path.join(WORKSPACE_ROOT, '.claude/logs/file-access.json');
const SESSION_FILE = path.join(WORKSPACE_ROOT, '.claude/logs/.current-session');

// Paths to track (Jarvis-specific)
const TRACKED_PATHS = [
  '.claude/context/',
  '.claude/commands/',
  '.claude/agents/',
...
function getRelativePath(filePath) {
  if (filePath.startsWith(WORKSPACE_ROOT)) {
    return filePath.substring(WORKSPACE_ROOT.length + 1);
  }
  return filePath;
}
