# Phase 7: Finalization

**Purpose**: Complete setup, document configuration, and archive setup files.

---

## Tasks

### 1. Generate Configuration Summary

Create `.claude/context/configuration-summary.md`:

```markdown
# AIfred Configuration Summary

**Configured**: [date]
**Version**: 2.2

## System
- **Host**: [hostname]
- **OS**: [os]
- **Docker**: [version or "not installed"]

## Environment Profile
- **Active Layers**: [general, homelab, development, ...]
- **Profile Config**: .claude/config/active-profile.yaml

## User Preferences
- **Automation Level**: [full/guided/manual]
- **Memory MCP**: [enabled/disabled]
- **Session Mode**: [automated/prompted/manual]
- **GitHub**: [enabled/disabled]

## Installed Components

### Task Management (Beads)
- Workspace: `.beads/`
- Config: `.beads/config.yaml`
- Actor hook: `.claude/hooks/beads-actor.sh`
- Shell aliases: `scripts/beads-aliases.sh`

### Hooks (from profile)
- [count] hooks across [count] events
- Generated by: `node scripts/profile-loader.js`

### Agents
- [list of deployed agents]

### Skills
- [list of enabled skills]

### Scheduled Jobs (Headless Claude)
- Dispatcher: `.claude/jobs/dispatcher.sh` (cron every 5 min)
- Active jobs: [list from registry.yaml]
- Personas: investigator, analyst, troubleshooter

### Cron Jobs (Legacy)
- [list from profile scripts.cron]

### MCP Servers
- [list if enabled]

## Discovered Infrastructure
- [list of discovered services]

## Managing Your Profile
- View: `/profile` or `node scripts/profile-loader.js --current`
- Add layer: `/profile add <layer>` (requires restart)
- Remove layer: `/profile remove <layer>` (requires restart)
- Available profiles: `/profile list`

## Next Steps
1. [Personalized recommendations]
2. [Based on active profile layers]
```

### 2. Update CLAUDE.md

Replace the "Project Status" section:

```markdown
## Project Status

**Setup Status**: ✅ Configured on [date]

### Configuration
- **Automation**: [level]
- **Focus**: [areas]
- **Memory**: [status]

### Quick Commands
- `/end-session` - End work cleanly
- `/discover <service>` - Document new services
- `/health-check` - System verification
```

### 3. Archive Setup Files

Move setup-phases to archive:

```bash
mkdir -p .claude/archive/setup
mv setup-phases .claude/archive/setup/
```

This keeps setup files available for reference but out of active context.

### 4. Update Session State

```markdown
## Current Work Status

**Status**: 🟢 Idle

**Current Task**: Setup complete

**Next Step**: Begin using AIfred - see knowledge/docs/getting-started.md
```

### 5. Register Existing Projects (Optional)

Ask user:

> "Would you like to register any existing code projects with AIfred?
>
> AIfred can track projects stored elsewhere (like ~/Code) without moving them.
> This enables context-aware assistance when you work on those projects.
>
> Options:
> - **Yes, register projects now**
> - **No, I'll add them later** (use `/register-project` anytime)"

**If yes, show discovered projects from Phase 1:**

```
I found these projects during discovery:

📁 ~/Code/
  1. [x] my-web-app (detected: typescript, web-app)
  2. [ ] old-project (detected: python)
  3. [x] api-service (detected: go, api)

Enter numbers to register, or provide a GitHub URL:
> 1, 3
> github.com/username/another-repo
```

**For each selected project:**

1. Add to `paths-registry.yaml` under `development.projects`
2. Create context file at `.claude/context/projects/<name>.md`
3. If GitHub URL: Clone to `projects_root` first

**If user provides a GitHub URL:**

```bash
# Clone to projects_root
cd [projects_root]  # e.g., ~/Code
git clone https://github.com/username/repo.git

# Auto-detect type/language
# Register in paths-registry.yaml
# Create context file
```

**Summary after registration:**

```
✅ Registered 3 projects:

1. my-web-app
   Path: ~/Code/my-web-app
   Context: .claude/context/projects/my-web-app.md

2. api-service
   Path: ~/Code/api-service
   Context: .claude/context/projects/api-service.md

3. another-repo (cloned from GitHub)
   Path: ~/Code/another-repo
   Context: .claude/context/projects/another-repo.md

When working on these projects, AIfred will automatically load
their context for better assistance.
```

### 6. Install Status Line (Optional)

Ask user:

> "Would you like to install the AIfred status line? It shows model, cost, project context, git branch, asset counts, Docker containers, memory, and context usage at the bottom of Claude Code."

**If yes:**

```bash
# Copy script to user's Claude config
cp scripts/statusline-command.sh ~/.claude/statusline-command.sh
chmod +x ~/.claude/statusline-command.sh
```

Then update `~/.claude/settings.json` to add:

```json
{
  "statusLine": {
    "type": "command",
    "command": "~/.claude/statusline-command.sh"
  }
}
```

**Customize project mappings:**

If the user registered projects in Task 5, populate `map_friendly_name()` in the installed script with their project directory-to-display-name mappings.

**Result**: Status line shows `ProjectName:SubProject` when working in registered code projects, plain `ProjectName` otherwise. Manual override via `echo "Name" > /tmp/claude-project-context`.

### 7. Git Commit

If git enabled:

```bash
git add -A
git commit -m "AIfred initial setup complete

Configured:
- Automation level: [level]
- Focus areas: [list]
- Hooks: [count] installed
- Agents: [count] deployed
- Memory MCP: [status]
- Projects registered: [count]

🤖 Generated with AIfred Setup Wizard"
```

### 8. Initialize Update Tracking

Initialize the AIfred component manifest so users can check for upstream updates later:

```bash
scripts/aifred-update.sh init
```

This creates `.aifred.yaml` (gitignored, user-specific) which tracks the version and SHA of every component. Users can later run `/stay-current check` or `scripts/aifred-update.sh check` to see if upstream has new versions.

Also creates `.aifred-ignore` from the template if it doesn't exist, where users can list components they never want updated.

### 9. GitHub Push (Optional)

If GitHub integration enabled:

```bash
git push -u origin main
```

---

## Final Summary

Present to user:

```
╔══════════════════════════════════════════════════════════════╗
║                    AIfred Setup Complete!                     ║
╠══════════════════════════════════════════════════════════════╣
║                                                               ║
║  Your AI infrastructure assistant is ready.                  ║
║                                                               ║
║  Configuration:                                               ║
║  • Automation: [level]                                        ║
║  • Focus: [areas]                                            ║
║  • Memory: [status]                                          ║
║  • Hooks: [count] active                                     ║
║  • Agents: [count] deployed                                  ║
║                                                               ║
║  Next Steps:                                                  ║
║  1. [Personalized first task]                                ║
║  2. [Based on discovered infrastructure]                     ║
║  3. Run /end-session when done today                         ║
║                                                               ║
║  Documentation: knowledge/docs/getting-started.md            ║
║                                                               ║
╚══════════════════════════════════════════════════════════════╝
```

---

## Cleanup

- [ ] Configuration summary created
- [ ] CLAUDE.md updated
- [ ] Setup files archived
- [ ] Session state updated
- [ ] Projects registered (or skipped)
- [ ] Status line installed (or skipped)
- [ ] Git commit created
- [ ] Update tracking initialized
- [ ] GitHub push (if enabled)
- [ ] Final summary presented

---

*Phase 7 of 8 (now 0-7) - Finalization - Setup Complete!*
