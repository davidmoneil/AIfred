...
# Note: 'all' also runs priority-cleanup.sh for documentation maintenance
#
# Created: 2025-12-26
...
check_logging() {
    ...
    AUDIT_LOG="${AIFRED_HOME:-$PWD}/.claude/logs/audit.jsonl"
    if [[ -f "$AUDIT_LOG" ]]; then
...
main() {
    ...
    # Run service-specific issue detection (only for full checks)
    if [[ "$TARGET_SECTION" == "all" ]]; then
        DETECT_SCRIPT="${AIFRED_HOME:-$PWD}/Scripts/detect-service-issues.sh"
        if [[ -x "$DETECT_SCRIPT" ]]; then
            log ""
            log "Running service issue detection..."
            "$DETECT_SCRIPT" 2>/dev/null && log "Service check completed - no issues." || log "Service check completed - issues detected (see detected-issues.yaml)."
        fi
    fi

    # Run priority cleanup check (only for full checks)
    if [[ "$TARGET_SECTION" == "all" ]]; then
        CLEANUP_SCRIPT="${AIFRED_HOME:-$PWD}/Scripts/priority-cleanup.sh"
        if [[ -x "$CLEANUP_SCRIPT" ]]; then
            log ""
            log "Running priority file cleanup check..."
            if "$CLEANUP_SCRIPT" --quiet 2>/dev/null; then
                log "Priority file is clean - no issues detected."
            else
                log "${YELLOW}Priority cleanup needed - run: $CLEANUP_SCRIPT${NC}"
            fi
        fi
    fi
